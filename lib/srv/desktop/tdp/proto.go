...
func decode(in peekReader, wantRaw bool) (Message, error) {
	...
	case TypeClientScreenSpec:
		if wantRaw {
			return decodeFixedSizeMessage(in, 8)
		}
	...
	case TypeMouseButton:
		if wantRaw {
			return decodeFixedSizeMessage(in, 2)
		}
		return decodeMouseButton(in)
	case TypeMouseWheel:
		if wantRaw {
			return decodeFixedSizeMessage(in, 3)
		}
		return decodeMouseWheel(in)
	case TypeKeyboardButton:
		if wantRaw {
			return decodeFixedSizeMessage(in, 5)
		}
...
func decodeFixedSizeMessage(in peekReader, n int) (rawMsg, error) {
	b := make([]byte, n)
	if _, err := io.ReadFull(in, b); err != nil {
		return nil, trace.Wrap(err)
	}
